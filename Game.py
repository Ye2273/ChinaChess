...
class Button(pygame.sprite.Sprite):
    def __init__(self, screen):
        self.screen = screen
        self.image = pygame.image.load("images/人机.jpg")
        self.rect = self.image.get_rect()
        self.rect.topleft = (615, 250)
        self.image1 = pygame.image.load("images/玩家.jpg")
        self.rect1 = self.image.get_rect()
        self.rect1.topleft = (615, 220)

    def draw(self):
        self.screen.blit(self.image, self.rect)
        self.screen.blit(self.image1, self.rect1)

    def clicked_exchange(self, chessboard: ChessBoard, event):
        if event.type == pygame.MOUSEBUTTONUP and self.rect.collidepoint(event.pos):
            return "AI"
        if event.type == pygame.MOUSEBUTTONUP and self.rect1.collidepoint(event.pos):
            return "玩家"
...
class RestartButton(pygame.sprite.Sprite):
    def __init__(self, screen):
        ...
        self.rect = self.image.get_rect()
        self.rect.topleft = (615, 170)

    def draw(self):        
        self.screen.blit(self.image, self.rect)

    def clicked_start(self, chessboard: ChessBoard, event):
        if event.type == pygame.MOUSEBUTTONUP and self.rect.collidepoint(event.pos):
            return True
...
class Game(object):
    ...
    def __init__(self, screen, chessboard):
        ...
        self.player_button = Button(screen)
        self.start_button = RestartButton(screen)
